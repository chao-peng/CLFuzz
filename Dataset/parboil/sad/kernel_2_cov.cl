...
__kernel void larger_sad_calc_8(__global unsigned short *blk_sad,
				  int mb_width,
				  int mb_height, __global int* ocl_kernel_branch_triggered_recorder, __global int* ocl_kernel_loop_recorder)
...
for (int search_pos = lidx; search_pos < (MAX_POS+1)/2 || (private_ocl_kernel_loop_boundary_not_reached[0] = false); search_pos += 32)
    {
