...
int GPU_expand(int idxL, int N1, int N2 , __local int* my_ocl_kernel_loop_recorder){ 
  return (idxL/N1)*N1*N2 + (idxL%N1); 
}      

__kernel void GPU_FFT_Global(int Ns, __global float* data0, __global float* data1, int N, __global int* ocl_kernel_loop_recorder) {__local int my_ocl_kernel_loop_recorder[2];
...
for( int r=0; r<R || (private_ocl_kernel_loop_boundary_not_reached[0] = false); r++ ) {
...
for( int r=0; r<R || (private_ocl_kernel_loop_boundary_not_reached[1] = false); r++ ){
