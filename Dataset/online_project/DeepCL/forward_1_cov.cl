...
__kernel void convolve_ints(global const int *p_imageSize, global const int *p_filterSize,
      global const int *image, global const int *filter, global int *result, __global int* ocl_kernel_loop_recorder) {__local int my_ocl_kernel_loop_recorder[2];
